...
> デモ動画をここに埋め込んでください（GIF または YouTube リンク）
...
### なぜ TensorFlow Lite を選んだか

普段の研究では PyTorch を使用していますが、Raspberry Pi のリソース制約（RAM・CPU）を考慮し、本プロジェクトでは **TensorFlow Lite** を採用しました。
推論時はフル TensorFlow をインストールせず `tflite_runtime` のみを使用することで、デバイス上のメモリ消費を最小限に抑えています。

### なぜ 3 層 CNN か

沸騰現象の特徴は鍋全体の形状ではなく、**局所的な気泡のテクスチャパターン**に現れます。
この性質から、深い ResNet や EfficientNet のような大規模アーキテクチャは過剰であり、
シンプルな 3 層 CNN による 2 値分類モデルが最適と判断しました。

---

## 直面した壁と学び

### カメラ画角の固定がもたらしたロバスト性の欠如

プロトタイプ段階では、学習を安定させるためにカメラを三脚で固定し、一定の画角でデータを収集しました。
この条件下では **val_AUC = 0.9999** という高精度を達成できましたが、
カメラを少しずらしたり、照明条件が変わったりするだけで誤検知が多発し、実運用には至りませんでした。

逆にカメラを動かしながらデータを収集すると、今度は学習が収束しないというジレンマに直面しました。

```
固定カメラでデータ収集  →  高精度（val_AUC ≈ 0.9999）だが汎化しない
カメラを動かして収集    →  多様な分布をカバーできるが学習が不安定
```

### 実環境の多様性との戦い

具材（卵など）の投入・照明の変化・鍋の種類の違いによっても誤検知が発生しました。
これらは事前に想定できていなかった「ドメインシフト」であり、
訓練データと推論環境の分布のズレが性能劣化の主因だと分析しています。

### この経験から得た最大の学び

> **モデルのアーキテクチャ以上に、「どのようなデータを学習させるか」というデータ中心のアプローチが実運用 AI の成否を決める。**

高精度な検証スコアは、あくまで「収集したデータの分布内での精度」にすぎません。
実世界で動くシステムを作るには、環境の多様性を意識したデータ収集設計とロバスト性の確保が不可欠であることを、
失敗を通じて身をもって学びました。
